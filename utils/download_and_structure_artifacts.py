    ...
    try:
        ...
        for model_and_tokenizer_name in MODELS_AND_TOKENIZERS_LIST:
            ...
            if os.path.exists(f"{cwd}{local_model_dir_path}{model_and_tokenizer_name}"):
                ...
                print(f"Model not found locally for {model_and_tokenizer_name}. Downloading from S3...")
                s3_client.download_file(s3_bucket, f"{s3_model_dir_path}{model_and_tokenizer_name}{file_type}", f"{cwd}{local_model_dir_path}{model_and_tokenizer_name}{file_type}")
                print(f"Model downloaded successfully.")
                os.system(f"{file_action} {cwd}{local_model_dir_path}{model_and_tokenizer_name}{file_type} -d {cwd}{local_model_dir_path}")
                os.system(f"rm {cwd}{local_model_dir_path}{model_and_tokenizer_name}{file_type}")
                print(f"{model_and_tokenizer_name} model is ready for use.")

            
            # Check if the tokenizer is available locally
            if os.path.exists(f"{cwd}{local_tokenizer_dir_path}{model_and_tokenizer_name}"):
                print(f"Tokenizer found locally for {model_and_tokenizer_name}.")
            else:
                print(f"Tokenizer not found locally for {model_and_tokenizer_name}. Downloading from S3...")
                s3_client.download_file(s3_bucket, f"{s3_tokenizer_dir_path}{model_and_tokenizer_name}{file_type}", f"{cwd}{local_tokenizer_dir_path}{model_and_tokenizer_name}{file_type}")
                print(f"Tokenizer downloaded successfully.")
                os.system(f"{file_action} {cwd}{local_tokenizer_dir_path}{model_and_tokenizer_name}{file_type} -d {cwd}{local_tokenizer_dir_path}")
                os.system(f"rm {cwd}{local_tokenizer_dir_path}{model_and_tokenizer_name}{file_type}")
